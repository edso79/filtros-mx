...
| 2026-08-31 | **Revisión mensual de agosto: 10 de 10 aportaciones siguen cubiertas**, verificado contra las listas descargadas ese día (EasyList Spanish `202608311832`). Ninguna de las 10 reglas ha desaparecido ni se ha roto. **Recordatorio del límite que el propio script declara:** comprueba que la regla siga en la lista, no que siga funcionando — eso lo dice la revisión trimestral abriendo los sitios. **Ese mismo día se enviaron DOS reportes nuevos:** [#368](https://github.com/easylist/easylistspanish/issues/368) (`codigoqro.mx`, tres reglas, 1,045,814 px²) y [#369](https://github.com/easylist/easylistspanish/issues/369) (`posta.com.mx`, prefijo `publicidad_`, 548,388 px²), las dos tras segunda carga con **las once filas idénticas al píxel** a seis días. Cuerpos verificados íntegros contra la API. **El proyecto vuelve a tener reportes abiertos: 12 enviados, 10 resueltos, 2 abiertos** | Revisión mensual + envío de #368 y #369 |
| 2026-09-01 | **#368 y #369 ACEPTADAS en un día, y las dos A MEDIAS** (commits `ed20593` y `4ac090f`, lista `202609011522`). Verificado contra la lista publicada **y contra el DOM**: de #368 entraron 2 de 3 reglas y falta `##.soft-news-banner` (**225,000 px² siguen visibles** en portada); de #369 entró `##.publicidad_posta` en vez del prefijo y faltan `.publicidad_ads` y `.publicidad_ads_box1` (**76,266 px²**). Las notas de los dos sitios quedan en **0 px²**. **El revisor pasa de 10 a 12 aportaciones vigiladas**, añadidas el mismo día de la aceptación; corrido contra `202609011532`: **12 de 12 ok**. **Los dos seguimientos se enviaron ese mismo día** y se verificaron contra la API: byte a byte idénticos, y las dos incidencias siguen cerradas | Verificación de aceptación + ampliación del revisor |
| 2026-09-21 | **Revisión mensual de septiembre: 12 de 12 aportaciones siguen cubiertas**, verificado contra EasyList Spanish `202609211542`. Incluye los desenlaces de septiembre: las 3 reglas de `posta.com.mx` (#369, cerrada del todo el 2-sep) y las 4 de `codigoqro.mx` (#368, cerrada del todo el 8-sep con `##.soft-news-banner`). **Recordatorio del límite:** comprueba que la regla siga en la lista, no que siga funcionando. **La revisión trimestral con ojos no se ha hecho nunca**: la primera vence hacia el 31-oct, tres meses después de la revisión inicial | Revisión mensual, automatizada |
